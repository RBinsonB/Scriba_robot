...
public:
  ...
  void callback(const scriba_msgs::data_odom& odom_data_msg)
  {
	...
	phi = (odom_data_msg.stepAngleSteerStart + odom_data_msg.stepAngleSteerEnd)/2; //Get average front wheel steer angle (in steps)
	phi = (phi * 0.12 * microstepSteeringRatio * pi)/180;										//Convert step to radians according according to microstepping configuration, gear ratio and stepper motor ratio
